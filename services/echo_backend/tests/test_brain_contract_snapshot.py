...
def json_pointer_diff(a: Any, b: Any, pointer: str = "") -> list[tuple[str, str, Any, Any]]:
    """Generate compact diff as list of (pointer, change_type, old, new) tuples."""
    diffs = []
    if type(a) != type(b):
        diffs.append((pointer or "/", "type_change", type(a).__name__, type(b).__name__))
    elif isinstance(a, dict):
        all_keys = set(a.keys()) | set(b.keys())
        for k in sorted(all_keys):
            child_ptr = f"{pointer}/{k}"
            if k not in a:
                diffs.append((child_ptr, "added", None, b[k]))
            elif k not in b:
                diffs.append((child_ptr, "removed", a[k], None))
            else:
                diffs.extend(json_pointer_diff(a[k], b[k], child_ptr))
    elif isinstance(a, list):
        if a != b:
            if len(a) != len(b):
                diffs.append((pointer, "length_change", len(a), len(b)))
            else:
                for i in range(len(a)):
                    diffs.extend(json_pointer_diff(a[i], b[i], f"{pointer}/{i}"))
    elif a != b:
        diffs.append((pointer, "value_change", a, b))
    return diffs
...
def test_brain_api_matches_snapshot(client, contract_snapshot_path):
    ...
    if current_hash != committed_hash:
        ...
        diagnostic = f"\nCompact diff (first 20 changes):\n"
        for ptr, change_type, old, new in diffs[:20]:
            if change_type == "added":
                diagnostic += f"  + {ptr}\n"
            elif change_type == "removed":
                diagnostic += f"  - {ptr}\n"
            elif change_type == "value_change":
                diagnostic += f"  ~ {ptr}: {old!r} -> {new!r}\n"
            else:
                diagnostic += f"  {change_type} {ptr}: {old} -> {new}\n"
        
        if len(diffs) > 20:
            diagnostic += f"  ... and {len(diffs) - 20} more changes\n"
