...
**Verifying Determinism**: After regenerating, run the generator a second time
and confirm `git diff` shows no changes. If changes appear on the second run,
the normalization logic has a bug that must be fixed before committing.

```bash
# Run twice to confirm determinism
python scripts/generate_brain_contract_v1_snapshot.py
python scripts/generate_brain_contract_v1_snapshot.py
git diff models/brain_contract_v1.json  # Should show no changes
```
...
**Why `separate_input_output_schemas=False`**:

FastAPI/Pydantic v2 can emit separate input/output schema variants for the same
model (e.g., `Message`, `Message-Input`, `Message-Output`). The order and presence
of these variants varies by environment, causing non-deterministic OpenAPI schemas.

To eliminate this source of drift, the FastAPI app is configured with:
...
This ensures:
- Each model has exactly ONE schema in the OpenAPI spec
- No `*-Input` or `*-Output` variants are emitted
- Contract hashes are deterministic across environments

**Array Sorting**: The normalization logic also sorts arrays that may appear in
non-deterministic order:
- `required` (field names)
- `enum` (allowed values)
- `anyOf`/`oneOf`/`allOf` (composite schemas)

This ensures identical JSON output regardless of Python dict/set ordering.
